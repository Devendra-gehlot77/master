...
const adminLogin = async (req, res) => {
  try {
    ...
    if (ifAdmin.password !== req.body.password)
      return res.status(400).json({ message: "Invalid Password" });

    const filepath = `${req.protocol}://${req.get(
      "host"
    )}/frankandoakservices/admin-panel/admin/`;
  ...
  try {
    ...
    if (req.files) {
      if (req.files.logo) {
        ...
        if (
          fs.existsSync(
            path.join(process.cwd(), "src", "uploads", "admin", oldData.logo)
          )
        ) {
          fs.unlinkSync(
            path.join(process.cwd(), "src", "uploads", "admin", oldData.logo)
      ...
      if (req.files.favicon) {
        ...
        if (
          fs.existsSync(
            path.join(process.cwd(), "src", "uploads", "admin", oldData.favicon)
          )
        ) {
          fs.unlinkSync(
            path.join(process.cwd(), "src", "uploads", "admin", oldData.favicon)
...
const updateEmail = async (req, res) => {
  try {
    ...
    if (!generatedOtp)
      return res
        .status(401)
        .json({ message: "OTP Expired! Generate OTP again" });
    if (generatedOtp !== Number(req.body.OTP))
      return res.status(403).json({ message: "Invalid OTP!" });
    const response = await AdminModel.findByIdAndUpdate(req.body._id, {
      email: req.body.newEmail,
    });

    res
      .status(200)
      .json({ message: "Email updated successfully", data: response });
  } catch (error) {
    console.log(error);
    res.status(500).json({ message: "Internal Server Error", error });
  }
